...
function findSupportedChangeTypeTestTypes(cb) {
  ...
  // type: fully specified mime type (and codecs substring if the bytestream format does not forbid codecs parameter). This is
  //   required for use with isTypeSupported, and if supported, should work with both addSourceBuffer and changeType (unless
  //   implementation has restrictions).
  //
  // relaxed_type: possibly ambiguous mime type/subtype without any codecs substring. This is the same as type minus any codecs
  //   substring.
  //
  // mime_subtype: the subtype of the mime type in type and relaxed_type. Across typeregistered in the bytestream format registry
  //   (https://www.w3.org/TR/mse-byte-stream-format-registry/), this is currently sufficient to describe uniquely which test media
  //   share the same bytestream format for use in implicit changeType testing.
  //
  // is_video: All test media currently is single track. This describes whether or not the track is video.
  //
  // url: Relative location of the test media file.
  //
  // The next two items enable more reliable test media splicing test logic that prevents buffered range gaps at the splice points.
  // start_time: Some test media begins at a time later than 0.0 seconds. This is the start time of the media.
  // keyframe_interval: Some test media contains out-of-order PTS versus DTS coded frames. In those cases, a constant
  //   keyframe_interval is needed to prevent severely truncating out-of-order GOPs at splice points.
